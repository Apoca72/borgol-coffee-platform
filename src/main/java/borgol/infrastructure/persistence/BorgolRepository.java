...
public class BorgolRepository {
    ...
    private void initSchema() {
        try (Statement s = conn().createStatement()) {
            ...
            // ── user_beans: хэрэглэгчийн кофены нөөц бүртгэл ───────────────
            s.execute("""
                CREATE TABLE IF NOT EXISTS user_beans (
                    ...
                    roasted_date  VARCHAR(20)      DEFAULT '',
                    notes         TEXT             DEFAULT '',
                    created_at    TIMESTAMP        DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (user_id) REFERENCES borgol_users(id) ON DELETE CASCADE
                )""");
    ...
    // ── UserBean (My Beans) CRUD ──────────────────────────────────────────────

    public List<UserBean> getUserBeans(int userId) {
        String sql = "SELECT * FROM user_beans WHERE user_id=? ORDER BY created_at DESC";
        List<UserBean> list = new ArrayList<>();
        try (PreparedStatement ps = conn().prepareStatement(sql)) {
            ps.setInt(1, userId);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) list.add(mapUserBean(rs));
            }
        } catch (SQLException e) { throw new RuntimeException(e); }
        return list;
    }

    public UserBean createUserBean(UserBean b) {
        String sql = """
            INSERT INTO user_beans (user_id,name,origin,roast_level,weight_g,purchase_date,roasted_date,notes)
            VALUES (?,?,?,?,?,?,?,?)""";
        try (PreparedStatement ps = conn().prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            ps.setDouble(5, b.getWeightG());
            ps.setString(6, nvl(b.getPurchaseDate()));
            ps.setString(7, nvl(b.getRoastedDate()));
    ...
    public UserBean updateUserBean(UserBean b) {
        String sql = """
            UPDATE user_beans SET name=?,origin=?,roast_level=?,weight_g=?,
            purchase_date=?,roasted_date=?,notes=?
            WHERE id=? AND user_id=?""";
        try (PreparedStatement ps = conn().prepareStatement(sql)) {
            ps.setString(1, nvl(b.getName()));
            ps.setString(2, nvl(b.getOrigin()));
            ps.setString(3, nvl(b.getRoastLevel()));
            ps.setDouble(4, b.getWeightG());
            ps.setString(5, nvl(b.getPurchaseDate()));
            ps.setString(6, nvl(b.getRoastedDate()));
            ps.setString(7, nvl(b.getNotes()));
            ps.setInt(8, b.getId());
            ps.setInt(9, b.getUserId());
            int updated = ps.executeUpdate();
            if (updated == 0) throw new IllegalArgumentException("Bean not found or not authorized");
        } catch (SQLException e) { throw new RuntimeException(e); }
        return b;
    }

    public boolean deleteUserBean(int id, int userId) {
        String sql = "DELETE FROM user_beans WHERE id=? AND user_id=?";
        try (PreparedStatement ps = conn().prepareStatement(sql)) {
            ps.setInt(1, id); ps.setInt(2, userId);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) { throw new RuntimeException(e); }
    ...
    private UserBean mapUserBean(ResultSet rs) throws SQLException {
        ...
        b.setRoastLevel(rs.getString("roast_level"));
        b.setWeightG(rs.getDouble("weight_g"));
        b.setPurchaseDate(rs.getString("purchase_date"));
        b.setRoastedDate(rs.getString("roasted_date"));
