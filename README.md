...
Веб интерфэйс (HTML / CSS / Vanilla JS)
  index.html | profile.html | brew-timer.html | admin.html
         ...
         HTTP REST (JSON) + SSE
                  |
         ┌────────────────────┐
         │   ApiGateway       │  ← CORS, rate limit (Redis INCR+EXPIRE),
         │   (before-filter)  │     request log, JWT/SOAP auth шийдвэрлэнэ
         └────────┬───────────┘
                  │
  ...
  GET  /api/notifications/stream ── SSE ──► Redis Pub/Sub fan-out
  ...
  BorgolService ─── eventBus.publish() ──► RedisEventBus
                                               │
                                    PUBLISH borgol:notify:{userId}
                                               │
                                           Redis Pub/Sub
                                               │
...
└── src/main/java/mn/edu/num/cafe/
    ...
    │   ├── cache/RedisClient        DCL Singleton, JedisPool
    │   └── messaging/RedisEventBus  Pub/Sub fan-out, SSE мэдэгдэл
    └── ui/
        ├── web/BorgolApiServer     REST endpoint, Bean AI, админ
        ├── web/ApiGateway          CORS, rate limit, auth (Gateway загвар)
